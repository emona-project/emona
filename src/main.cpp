...
#if defined(NDEBUG)
# error "Emona cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Emona Signed Message:\n";
...
{
    // Emona: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Emona: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Emona: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "PL Times 27/jan/2022 Poland";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EmonaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EmonaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EmonaMiner(CWallet *pwallet)
{
    printf("EmonaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("emona-miner");
    ...
    try { loop {
        ...
        printf("Running EmonaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EmonaMiner, pwallet));
}
